...
const SingleLink = ({links, countries}: Props) => {
  const {id} = useParams()
  const linkFiltered: Urls[] | null =links && links.filter(link => link.id.toString() === id)
  const link: Urls | null = linkFiltered && linkFiltered[0]

  const dataAnalytics:Data[] = []
  countries && link?.linkCountries.forEach((c) =>{
  ...
  return link ? (<div className={s.containerSection} >
