...
const Card = ({ link }: Props) => {
  const handleClipboard = () =>
    navigator.clipboard.writeText(
      `https://api-shortener.onrender.com/${link.shortUrl}`
    );
        ...
        <div>
          <Link to={`/dashboard/links/single/${link.id}`}>
            <h2 className={s.cardTitle}>
              {link.name?.length < 1 ? "untitled" : link.name}
            </h2>
          </Link>
          <a
            className={s.shorted}
            target="blanc"
            href={`https://api-shortener.onrender.com/${link.shortUrl}`}
          >{`https://api-shortener.onrender.com/${link.shortUrl}`}</a>
          <a className={s.url} target="blanc" href={link.url}>
            {link.url}...
          </a>

          <div className={s.tagsContainer}>
            <Link to={`/dashboard/links/single/${link.id}`}>
              <span className={`${link.clicks > 0 ? s.isEngagement : ""}`}>
                <OrbIcon />
            ...
            <span>
              <CalendarIcon />
              {link.createdAt &&
                link.createdAt.slice(0, 10).split("-").reverse().join("/")}
            </span>
          </div>
        </div>
      </div>
